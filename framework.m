	...
	elseif(options.speechRecognition.doGetRemoteResults)
		...
		results.speechRecognition = ...
				speechRecognizer(options.speechRecognition.sigDir, ...
						   options.tmpDir, ...
						   options.speechRecognition.db, ...
						   options.speechRecognition.model);
   end%do remote
end
%%%%%speech recognition%%%%%
...
ret = false;
if(options.doDma|options.doDistanceFiltering|options.doDistanceGate|...
		options.doTwinMicBeamforming|options.doTwinMicNullSteering|...
